...
没有命令行也行：服务详情页「更多 → 结算归档」就是这一条，弹窗里填版本号。一样的规矩 ——
**在停服之前点**。

结算过的版本不会从看板上消失：详情页右上角的版本下拉能切回任一历史版本，看它结算时的
总覆盖、新增代码明细乃至逐行源码（源码片段在结算那一刻就存进了归档，之后源码目录换成
新版也不影响）。项目面板上的「报表」则按时间范围把项目下所有服务的结算版本、单测报告
列在一页，可导出 CSV 或打印成 PDF 交差。
