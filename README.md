...
看板是独立的 Vue 前端（产物在 `covhub/webui/`，随包分发，hub 机器不装 node）：首页按项目分组
列出全部服务的在线状态、版本、运行时总/新增覆盖率、单测总/新增覆盖率、触达类数，顶部可以
切换项目、新建 / 编辑 / 删除项目、把服务归到项目里；点进服务是详情页：四个大数字、趋势图、新增代码按文件的覆盖明细（含未覆盖行号）、已结算版本、单测历史，
